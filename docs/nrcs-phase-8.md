...
For a completed dry-run report with tag collisions, use **Tag Collision Resolution**: select a legacy CMS tag (for example `ucsd`), choose the intended existing canonical NRCS tag, then click **Add Mapping**. Repeat for the other tags, then click **Save Mappings & Create Refreshed Dry Run** once. Pending selections are browser-local until that save; leaving/reloading the page or switching runs discards them. All submitted choices are validated before a new run is created. Use **Resume Processing** and review exceptions before authorizing import. Previously selected owner/classification mappings are retained. **Refresh Tags** reloads the canonical list without rerunning the migration; canonical tags are fetched in database pages rather than truncated at 1,000 rows. Categories, aliases, and school activity terms are not separate canonical tag choices.
