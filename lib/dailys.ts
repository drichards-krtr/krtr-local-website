...
export const getLatestPublishedDaily = cache(async function getLatestPublishedDaily(
  ...
  noStore();
  const { data: district, error: districtError } = await supabase.from("districts").select("timezone").eq("district_key", districtKey).single();
  if (districtError || !district?.timezone) throw new Error(`Cannot load Daily timezone: ${districtError?.message || "missing timezone"}`);
  const today = districtCalendarDay(district.timezone, new Date());
  const { data, error } = await supabase
    .from("dailys")
    .select(PUBLIC_DAILY_SELECT)
    .eq("district_key", districtKey)
    .eq("status", "published")
    .not("nrcs_source_id", "is", null)
    .eq("nrcs_publication_date", today)
