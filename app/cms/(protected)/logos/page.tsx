...
function revalidateLogoPaths(districtKey: DistrictKey) {
  revalidatePath("/", "layout");
  revalidatePath("/cms/logos");
  revalidatePath(`/cms/logos?district=${districtKey}`);
}
...
export default async function LogosPage({
  ...
  async function addLogo(formData: FormData) {
    ...
    revalidateLogoPaths(nextDistrictKey);
    redirect(`/cms/logos?district=${nextDistrictKey}`);
  }
  ...
  async function toggleLogo(formData: FormData) {
    ...
    const nextDistrictKey = parseDistrictKey(String(formData.get("district_key") || "")) || districtKey;
    await service.from("logos").update({ active: next }).eq("id", id).eq("district_key", districtKey);
    revalidateLogoPaths(nextDistrictKey);
    redirect(`/cms/logos?district=${nextDistrictKey}`);
  }
  ...
  return (
    <div className="grid gap-6">
      <header>
        ...
        <p className="text-sm text-neutral-500">Manage scheduled header logos with date windows.</p>
        <p className="mt-1 text-sm text-neutral-600">Editing {district.name}.</p>
        <p className="mt-2 text-sm text-neutral-600">Recommended logo size: 480w x 120h (or larger at the same 4:1 ratio).</p>
      ...
      <section className="rounded border border-neutral-200 bg-white p-4">
        ...
        <form action={addLogo} className="grid gap-3 md:grid-cols-2">
          ...
          <p className="text-xs text-neutral-500 md:col-span-2">
            Dates are for scheduled logos. Leave them blank only when marking a logo as the default fallback.
          </p>
      ...
      <section className="rounded border border-neutral-200 bg-white">
        ...
        {(logos || []).map((logo) => (
          ...
          >
            ...
            <div>{logo.is_default ? "Fallback" : `${logo.start_date} - ${logo.end_date}`}</div>
            <div>{logo.active ? "Active" : "Inactive"}</div>
            <div className="truncate">{logo.image_url || "-"}</div>
            <div className="flex gap-3">
              <a href={`/cms/logos/${logo.id}?district=${districtKey}`} className="text-sm underline">
                Edit
              </a>
              <form action={toggleLogo}>
                <input type="hidden" name="id" value={logo.id} />
                <input type="hidden" name="district_key" value={districtKey} />
                <input type="hidden" name="next" value={(!logo.active).toString()} />
