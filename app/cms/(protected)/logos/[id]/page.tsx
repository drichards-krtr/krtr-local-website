...
export default async function EditLogoPage({ params }: { params: { id: string } }) {
  ...
  async function updateLogo(formData: FormData) {
    ...
    redirect(`/cms/logos?district=${encodeURIComponent(nextDistrictKey)}`);
  }
  ...
  return (
    <div className="grid gap-6">
      ...
      <form action={updateLogo} className="grid gap-3 rounded border border-neutral-200 bg-white p-6 md:grid-cols-2">
        <select name="district_key" defaultValue={logo.district_key} className="rounded border border-neutral-300 px-3 py-2 text-sm">
        ...
        <div className="md:col-span-2">
          <ImageUploadField name="image_url" label="Logo Image" folder="krtr/logos" initialUrl={logo.image_url || ""} />
        </div>
