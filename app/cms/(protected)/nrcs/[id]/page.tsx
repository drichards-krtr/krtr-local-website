...
import { planWebProjection, publicationEligibility } from "@/lib/nrcsRehearsal";
...
export default async function NrcsPreview({ params }: { params: Promise<{ id: string }> }) {
  ...
  let webPlan: ReturnType<typeof planWebProjection> | null = null;
  if (envelope.kind === "web") {
    // RLS-protected read-only rehearsal; never modify public records.
    const filters = [`nrcs_output_id.eq.${envelope.source_id}`, `nrcs_story_id.eq.${envelope.payload.story_id}`];
    if (envelope.payload.slug) filters.push(`slug.eq.${envelope.payload.slug}`);
    if (envelope.payload.cms_article_id) filters.push(`id.eq.${envelope.payload.cms_article_id}`);
    const { data: candidates, error: matchError } = await supabase.from("stories").select("id,district_key,slug,nrcs_output_id,nrcs_story_id").eq("district_key", envelope.district_key).or(filters.join(",")).limit(10);
    if (matchError) throw new Error(`Cannot rehearse CMS article resolution: ${matchError.message}`);
    webPlan = planWebProjection(envelope, candidates || []);
  }
  const eligibility = publicationEligibility(envelope);
  ...
  return <div className="mx-auto grid max-w-4xl gap-5"><Link href="/cms/nrcs" className="w-fit text-sm underline">Back to NRCS Receipts</Link><h1 className="text-2xl font-semibold">Non-Public Preview</h1><p className="border-l-4 border-yellow-500 bg-yellow-50 p-3 text-sm">{row.district_key} · {row.kind} · Revision {row.revision} · Received, not published</p>
    ...
    {envelope.kind === "daily" && <section className="grid gap-4"><h2 className="text-xl font-semibold">{envelope.payload.title}</h2><p className="text-sm">{envelope.payload.status} · {envelope.payload.scheduled_at} · {envelope.payload.timezone}</p>{envelope.payload.asset.asset_type === "video" ? <MuxPlayer playbackId={envelope.payload.asset.playback_id} poster={envelope.payload.asset.thumbnail_url} orientation={envelope.payload.asset.orientation} /> : <img className="aspect-video w-full object-contain" alt={envelope.payload.asset.title} src={envelope.payload.asset.url} />}</section>}
    <section className="border-t pt-4"><h2 className="mb-2 font-semibold">Publication Rehearsal</h2><dl className="grid gap-2 text-sm"><dt>Eligible now: {eligibility.visible ? "Yes" : "No"}</dt><dt>Effective date/time: {eligibility.effectiveAt || "None"}</dt>{webPlan && <><dt>Article action: {webPlan.action.replaceAll("_", " ")}</dt><dt className="break-all">Existing CMS article: {webPlan.cmsArticleId || "None"}</dt><dt className="break-all">Planned public path: {webPlan.publicPath || "None"}</dt></>}</dl>{webPlan?.problems.map(problem => <p key={problem} role="alert" className="mt-2 text-sm text-red-800">{problem}</p>)}</section>
