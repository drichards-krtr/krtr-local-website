...
export default async function NrcsPreview({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="mx-auto grid max-w-4xl gap-5"><Link href="/cms/nrcs" className="w-fit text-sm underline">Back to NRCS Receipts</Link><h1 className="text-2xl font-semibold">Non-Public Preview</h1><p className="border-l-4 border-yellow-500 bg-yellow-50 p-3 text-sm">{row.district_key} · {row.kind} · Revision {row.revision} · Received, not published</p>
    ...
    {envelope.kind === "daily" && <section className="grid gap-4"><h2 className="text-xl font-semibold">{envelope.payload.title}</h2><p className="text-sm">{envelope.payload.status} · {envelope.payload.scheduled_at} · {envelope.payload.timezone}</p><img className="aspect-video w-full object-contain" alt={envelope.payload.hero.title} src={envelope.payload.hero.url} /><MuxPlayer playbackId={envelope.payload.video.playback_id} poster={envelope.payload.hero.url} orientation={envelope.payload.video.orientation} /></section>}
