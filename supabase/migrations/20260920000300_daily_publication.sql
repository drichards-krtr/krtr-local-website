...
alter table public.dailys add column if not exists nrcs_source_id uuid unique;
alter table public.dailys add column if not exists nrcs_revision integer;
alter table public.nrcs_publication_projections drop constraint if exists nrcs_publication_projections_kind_check;
alter table public.nrcs_publication_projections add constraint nrcs_publication_projections_kind_check check(kind in ('web','homepage','alert','daily'));

create function public.receive_nrcs_daily_publication(p_package jsonb,p_hash text) returns jsonb
language plpgsql security invoker set search_path=public as $$
declare receipt jsonb; projection public.nrcs_publication_projections; p jsonb; media jsonb; zone text; daily_id uuid; desired text; local_date date;
begin
  ...
  p:=projection.package->'payload'; media:=p->'asset'; desired:=p->>'status';
  ...
  values(projection.district_key,p->>'title',case when desired='archived' then 'archived' when desired='draft' then 'draft' else 'published' end,(p->>'scheduled_at')::timestamptz at time zone 'UTC','nrcs-daily-'||(p_package->>'source_id'),case when media->>'asset_type'='video' then media->>'thumbnail_url' else media->>'url' end,media->>'public_id',media->>'mux_asset_id',media->>'playback_id',case when media->>'asset_type'='video' then 'ready' else null end,coalesce(media->>'orientation','horizontal'),(p_package->>'source_id')::uuid,projection.revision,(p->>'edition_id')::uuid,local_date,zone)
