...
export default async function HomepagePage({ searchParams }: { searchParams: Promise<{ district?: string; story?: string; event?: string }> }) {
  ...
  const [lineup, alerts, currentAlert, alertDeliveries] = await Promise.all([
    supabase.from("nrcs_homepage_lineups").select("*").eq("district_key", district.district_key).maybeSingle(),
    supabase.from("nrcs_priority_alerts").select("*").eq("district_key", district.district_key).order("updated_at", { ascending: false }).limit(100),
    supabase.from("nrcs_priority_alerts").select("*").eq("district_key", district.district_key).is("archived_at", null).eq("active", true).or(`start_at.is.null,start_at.lte.${now}`).or(`end_at.is.null,end_at.gt.${now}`).limit(1),
    supabase.from("nrcs_publication_deliveries").select("source_id").eq("district_key", district.district_key).eq("kind", "alert"),
  ]);
