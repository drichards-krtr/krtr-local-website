...
function AlertEditor({ districtKey, timezone, alert, target, onSaved }: { districtKey: string; timezone: string; alert: PriorityAlert | null; target: Target; onSaved: (a: PriorityAlert) => void }) {
  ...
  </fieldset></form>{alert && <NrcsPublicationDelivery kind="alert" sourceId={alert.id} districtKey={districtKey} revision={alert.revision} disabled={state.dirty || state.busy} />}</div>;
}
export default function NrcsHomepageManager({ districtKey, timezone, initialLineup, initialAlerts, initialTarget }: { districtKey: string; timezone: string; initialLineup: HomepageLineup | null; initialAlerts: PriorityAlert[]; initialTarget: Target }) {
  ...
  </fieldset></form>{edition && <div className="grid gap-3"><h3 className="text-sm font-semibold">Daily Edition Images</h3><fieldset disabled={imageState.busy || mediaState.busy || state.busy}><NrcsCloudinaryAssetPicker clientSubmit storyId="" districtKey={districtKey} action={async form => { const data = await imageState.save("image", { ...Object.fromEntries(form), edition_id: edition, revision: 0 }); if (data) { change(() => setAsset(data.asset.id)); setMediaRefresh(k => k + 1); } }} /></fieldset><SaveFeedback state={imageState} /></div>}{lineup && <p className="text-sm text-neutral-600">Saved Daily: {lineup.daily_publication_date || "None"}{lineup.daily_publication_date && now ? dailyIsEligible(lineup.daily_publication_date, timezone, now) ? " · Eligible today" : " · Not eligible today" : ""}</p>}</section>
