...
const canonicalTag = { id: source, name: "La Porte City", slug: "lpc", tag_type: "place" };
const tagged = aliases => ({ ...envelope, payload: { ...envelope.payload, tags: [{ ...canonicalTag, ...aliases }] } });
assert.deepEqual(contract.validatePublication(tagged({})), tagged({}), "Historical packages must not gain fields or change hashes");
assert.deepEqual(contract.validatePublication(tagged({ aliases: ["la-porte-city"] })), tagged({ aliases: ["la-porte-city"] }));
assert.throws(() => contract.validatePublication(tagged({ aliases: ["invalid/slug"] })), /alias/);
assert.throws(() => contract.validatePublication(tagged({ aliases: ["lpc", "lpc"] })), /Duplicate/);
const taxonomy = load("lib/publicTaxonomy.ts");
assert.equal(taxonomy.findPublicTag([{ ...canonicalTag, aliases: ["la-porte-city"] }], "la-porte-city").slug, "lpc");
assert.equal(taxonomy.findPublicTag([{ ...canonicalTag }], "unrelated"), null);
