...
const fallbackStory = migration.normalizeLegacy("stories", story, null, new Date(), "America/Chicago", {}, admin);
assert.equal(fallbackStory.normalized.owner_id, admin);
assert.equal(fallbackStory.normalized.mapping.fallback_author_email, "drichards@krtrlocal.tv");
assert.equal(migration.normalizeLegacy("stories", story, contributor, new Date(), "America/Chicago", {}, admin).normalized.owner_id, contributor);
assert.equal(migration.normalizeLegacy("stories", story, null, new Date(), "America/Chicago", { owner_id: contributor }, admin).normalized.owner_id, contributor);
assert.equal(migration.normalizeLegacy("stories", story, null, new Date(), "America/Chicago", { owner_id: null }, admin).normalized.owner_id, admin);
assert.equal(migration.normalizeLegacy("events", { id: eventSource }, null, new Date(), "America/Chicago", {}, admin).normalized.owner_id, admin);
...
(async () => {
  ...
  try {
    ...
    const canonical = (await db.query("insert into nrcs_tags(name,slug) values('Union Community School District','ucsd') returning id")).rows[0].id;
    await db.query("update nrcs_migration_runs set lease_until=null,lease_token=null where id=$1", [fourth]);
    const tagRaw = { district_key: "dlpc", slug: "ucsd", name: "Union Schools" };
    const snapshot = (await db.query("select nrcs_migration_tag_snapshot('ucsd',$1) choice", [canonical])).rows[0].choice;
    const tagMapping = { tags: { ucsd: snapshot } };
    assert.equal(await apply(fourth, await stage(fourth, "tags", tagRaw, tagMapping)), "imported");
    assert.equal((await db.query("select name from nrcs_tags where id=$1", [canonical])).rows[0].name, "Union Community School District", "Explicit mapping must preserve the canonical name");
    const mappedStory = migration.normalizeLegacy("stories", { ...story, tags: ["ucsd"], tag_definitions: [{ slug: "ucsd", name: "Union Schools" }] }, admin, new Date(), "America/Chicago", tagMapping);
    assert.equal(mappedStory.normalized.tag_definitions[0].name, snapshot.name);
    await db.exec("update nrcs_tags set name='Updated canonical label' where slug='ucsd'");
    await db.query("delete from nrcs_migration_items where run_id=$1 and kind='tags' and source_id='ucsd'", [fourth]);
    assert.equal(await apply(fourth, await stage(fourth, "tags", tagRaw, tagMapping)), "conflict", "Changed canonical choices require reconfirmation");
    const other = (await db.query("insert into nrcs_tags(name,slug) values('Other Organization','other-organization') returning id")).rows[0].id;
    await assert.rejects(db.query("select nrcs_migration_tag_snapshot('ucsd',$1)", [other]), /already belongs/);
    const aliasChoice = (await db.query("select nrcs_migration_tag_snapshot('legacy-organization',$1) choice", [other])).rows[0].choice;
    assert.equal(await apply(fourth, await stage(fourth, "tags", { district_key: "dlpc", slug: "legacy-organization", name: "Legacy Organization" }, { tags: { "legacy-organization": aliasChoice } })), "imported");
    assert.equal((await db.query("select tag_id from nrcs_tag_aliases where alias='legacy-organization'")).rows[0].tag_id, other);
